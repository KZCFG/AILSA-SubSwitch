...
enum UsageProgressSkin: String, CaseIterable {
    case official, ailsa

    static func resolve(_ style: UsageProgressFillStyle, rawValue: String) -> UsageProgressFillStyle {
        rawValue == Self.ailsa.rawValue ? .monochrome : style
    }
}

/// Shared by bars, rings and quota-history columns. These are display choices
/// only and deliberately do not modify stored quota or switching decisions.
private struct UsageProgressSkinPreferences: DynamicProperty {
    @AppStorage("ass.progressSkin.codex") private var codex = "official"
    @AppStorage("ass.progressSkin.antigravity") private var antigravity = "official"
    @AppStorage("ass.progressSkin.cursor") private var cursor = "official"
...
struct LiquidProgressBar: View {
    ...
    private var skins = UsageProgressSkinPreferences()
...
/// Compact rings use a flat provider tint or the selected monochrome skin.
        ...
        ZStack {
            ...
            if metrics.trimEnd > 0 {
                RingFill(
                    ...
                    tint: QuotaBrandPalette.flatTint(for: skins.resolve(fillStyle), fallback: tint),
                    metrics: metrics
                )
            }
        }
