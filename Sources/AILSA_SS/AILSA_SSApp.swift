...
struct AILSA_SSApp {
    ...
    // visibility/position state from AILSA_SS's application identity.
...
final class SubSwitchApplicationDelegate: NSObject, NSApplicationDelegate {
    ...
    func showQuotaWindow() {
        ...
        if quotaWindow == nil, let content = Self.makeQuotaContent?() {
            ...
            window.title = AILSA_SSApp.quotaWindowTitle
    ...
    /// Recreates the status item after a menu-bar organizer has retained a
    /// stale host identity or moved the item to a hidden section. This is a
    /// best-effort repair: the organizer still owns the final placement.
    @discardableResult
    func repairStatusItemDisplay() -> Bool {
        popover.performClose(nil)
        if let statusItem {
        ...
        statusItem?.button?.image = AILSA_SSApp.makeMenuBarSymbolImage()
        statusItem?.button?.setAccessibilityLabel(AILSA_SSApp.productName)
        return statusItem?.isVisible == true && statusItem?.button?.image?.isValid == true
    }
    ...
    private func installStatusItem() {
        ...
        if let button = item.button {
            button.image = AILSA_SSApp.makeMenuBarSymbolImage()
            button.toolTip = AILSA_SSApp.productName
            button.setAccessibilityLabel(AILSA_SSApp.productName)
            button.target = self
