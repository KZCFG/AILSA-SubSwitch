...
extension AccountsPageModel {
    func switchAccount(id: String) async {
        ...
        do {
            ...
            var accounts = switchResult.accounts
            let selectedAccount = switchResult.selectedAccount
            if selectedAccount.provider != .cursor,
               let settingsCoordinator,
               let settings = try? await settingsCoordinator.currentSettings(),
               settings.quotaWindowActivationMode == .activeAccount {
                // Checking quota does not activate a window. The refreshed
                // provider usage determines whether a countdown may run.
                if let checked = try? await coordinator.checkQuotaWindows(accountIDs: [selectedAccount.id]) {
                    accounts = checked
                }
            }
            AccountSwitchDebugLog.write(
    ...
    func smartSwitch() async {
        do {
            ...
            var accounts = try await coordinator.listAccounts(refreshWorkspaceMetadata: false)
            let selectedAccount = switchResult.0
            if selectedAccount.provider != .cursor,
               let settingsCoordinator,
               let settings = try? await settingsCoordinator.currentSettings(),
               settings.quotaWindowActivationMode == .activeAccount,
               let checked = try? await coordinator.checkQuotaWindows(accountIDs: [selectedAccount.id]) {
                accounts = checked
            }
            AccountSwitchDebugLog.write(
    ...
    func toggleAllAccountsCollapsed() {
        ...
        if collapsedAccountIDs.isSuperset(of: ids) { collapsedAccountIDs.subtract(ids) }
        else { collapsedAccountIDs.formUnion(ids) }
    }
