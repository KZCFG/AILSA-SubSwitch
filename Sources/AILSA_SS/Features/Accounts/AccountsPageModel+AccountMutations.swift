...
extension AccountsPageModel {
    private var authFlowLogger: Logger {
        Logger(subsystem: "AILSA_SS", category: "AccountsPageAuthFlow")
    }
    ...
    func addAccountViaLogin() async {
        ...
        addAccountProgressStage = selectedProvider == .antigravity ? .starting : nil
        let stageHandler: (@Sendable @MainActor (AccountAddProgressStage) async -> Void)?
        if selectedProvider == .antigravity {
            stageHandler = { @MainActor [weak self] (stage: AccountAddProgressStage) async in
                self?.addAccountProgressStage = stage
            }
        } else {
            stageHandler = nil
        }
        let task = Task { [coordinator] in
            if selectedProvider == .antigravity {
                return try await coordinator.addAntigravityAccountViaNativeLogin(
                    customLabel: nil,
