...
final class SettingsPageModel: ObservableObject {
    ...
    @Published private(set) var quotaDisplayAccounts: [QuotaDisplayAccount] = []
    ...
    func updateQuotaVisibilityCatalog(from accounts: [AccountSummary]) {
        let nextAccounts = accounts.map { QuotaDisplayAccount(account: $0) }
        if nextAccounts != quotaDisplayAccounts { quotaDisplayAccounts = nextAccounts }
...
struct QuotaDisplayAccount: Equatable, Identifiable {
    let id: String
    let provider: AccountProvider
    let name: String
    let choices: [CompactQuotaChoice]

    init(account: AccountSummary) {
        id = account.id
        provider = account.provider
        name = account.email ?? (account.label.isEmpty ? account.id : account.label)
        let presentation = AccountCardPresentation(account: account, isCollapsed: true,
            locale: L10n.currentLocale, usageProgressDisplayMode: .remaining)
        choices = presentation.availableCompactItems.map(CompactQuotaChoice.init)
    }
}

extension Notification.Name {
    static let ailsaSSOpenQuotaDisplaySettings = Notification.Name(
