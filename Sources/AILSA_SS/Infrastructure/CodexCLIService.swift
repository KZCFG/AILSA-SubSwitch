...
final class CodexCLIService: CodexCLIServiceProtocol, @unchecked Sendable {
    ...
    private func findCodexAppPath() -> URL? {
        ...
        if let spotlightChatGPT = spotlightFindApp(named: "ChatGPT.app") {
            return spotlightChatGPT
        }
        return nil
    }
    ...
    private func isCodexProcessRunning() -> Bool {
        ...
        let chatgpt = try? CommandRunner.run("/usr/bin/pgrep", arguments: ["-x", "ChatGPT"])
        return chatgpt?.status == 0
    }
