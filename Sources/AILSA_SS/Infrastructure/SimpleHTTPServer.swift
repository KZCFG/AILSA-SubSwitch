...
final class SimpleHTTPServer: @unchecked Sendable {
    ...
    init(port: UInt16, handler: @escaping RequestHandler) throws {
        ...
        self.queue = DispatchQueue(label: "ailsa.subswitch.oauth.loopback.listener", qos: .userInitiated)
    ...
    /// Hard cap so a misbehaving local client cannot make the loopback
    /// OAuth-callback server buffer without bound.
