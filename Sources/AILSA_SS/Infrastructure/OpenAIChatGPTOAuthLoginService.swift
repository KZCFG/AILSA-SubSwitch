...
final class OpenAIChatGPTOAuthLoginService: ChatGPTOAuthLoginServiceProtocol, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "AILSA_SS", category: "AuthLogin")
            ...
            if let code = params["code"], !code.isEmpty {
                ...
                Task {
                    do {
                        ...
                        Logger(subsystem: "AILSA_SS", category: "AuthLogin").log("OAuth token exchange succeeded")
                        AuthFlowDebugLog.write("AuthLogin", "OAuth token exchange succeeded")
                        callback.succeed(tokens)
                    } catch {
                        Logger(subsystem: "AILSA_SS", category: "AuthLogin").error("OAuth token exchange failed: \(error.localizedDescription, privacy: .public)")
