...
enum AccountIdentity {
    ...
    static func preferredMatchIndex(
        ...
        if let exactKeyIndex = accounts.firstIndex(where: {
            $0.provider == extracted.provider && key(for: $0) == extractedKey
        }) {
        ...
        let wildcardMatches = accounts.indices.filter {
            accounts[$0].provider == extracted.provider
    ...
    /// AntiGravity's older cards used Google's opaque subject as accountID,
    /// while the native credential reader now uses its identity-verified
    /// e-mail as the stable accountID.  A generic account-key comparison is
    /// deliberately unable to bridge those two representations.  Callers
    /// must use this only after the incoming e-mail was verified by the native
    /// session or the profile-bound OAuth/userinfo flow.
    static func antigravityEmailMatchIndices(
        for extracted: ExtractedAuth,
        in accounts: [StoredAccount]
    ) -> [Int] {
        guard extracted.provider == .antigravity,
              let verifiedEmail = normalizedEmail(extracted.email)
        else {
            return []
        }
        return accounts.indices.filter { index in
            accounts[index].provider == .antigravity
                && normalizedEmail(accounts[index].email) == verifiedEmail
        }
    }
