...
extension AccountsPageModel {
    ...
    func applySettings(_ settings: AppSettings) {
        codexSmartSwitchEnabled = settings.autoSmartSwitch
        antigravitySmartSwitchEnabled = settings.autoSmartSwitchAntigravity
        usageProgressDisplayMode = settings.usageProgressDisplayMode
        quotaVisibilityPreferences = settings.quotaVisibility
    }
    ...
    func load() async {
        do {
            if let settingsCoordinator { applySettings(try await settingsCoordinator.currentSettings()) }
