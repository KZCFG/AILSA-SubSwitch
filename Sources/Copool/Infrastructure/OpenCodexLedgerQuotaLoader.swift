...
struct OpenCodexLedgerQuotaLoader: Sendable {
    let ledgerURL: URL
    let reader: OpenCodexLedgerMeteringReader
    ...
    init(
        ...
        reader = OpenCodexLedgerMeteringReader(pricingCatalog: pricingCatalog)
    ...
    init(
        ...
        let catalog = verifiedPricingCatalogURL.flatMap {
            try? OpenCodexVerifiedPricingCatalogLoader.load(url: $0)
        } ?? .empty
        reader = OpenCodexLedgerMeteringReader(pricingCatalog: catalog)
...
struct OpenCodexIncrementalLedger: Sendable {
    private var state = OpenCodexLedgerMeteringReader.State()
    private var offset: UInt64 = 0
    private var identity: String?
    private var boundary = Data()
    private var reader: OpenCodexLedgerMeteringReader?
    ...
    private mutating func readFile(url: URL, catalogURL: URL?, compact: Bool) throws {
        ...
        if changed || reader == nil {
            ...
            let catalog = catalogURL.flatMap { try? OpenCodexVerifiedPricingCatalogLoader.load(url: $0) } ?? .empty
            reader = OpenCodexLedgerMeteringReader(pricingCatalog: catalog)
        }
