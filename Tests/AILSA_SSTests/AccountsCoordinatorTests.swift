...
final class AccountsCoordinatorTests: XCTestCase {
    ...
    func testListAccountsBackfillsWorkspaceNameFromRemoteMetadata() async throws {
        ...
        let coordinator = AccountsCoordinator(
            ...
            opencodeAuthSyncService: StubAILSA_SSAuthSyncService(),
    ...
    func testAutoSmartSwitchReturnsCurrentAccountAfterSwitch() async throws {
        ...
        var automaticSettings = AppSettings.defaultValue
        automaticSettings.autoSmartSwitch = true
        let coordinator = AccountsCoordinator(
            storeRepository: storeRepository,
            settingsRepository: TestSettingsRepository(settings: automaticSettings),
            authRepository: MultiAccountAuthRepository(
                extractedByAccountID: [
                    "account-1": makeExtractedAuth(accountID: "account-1"),
                    "account-2": makeExtractedAuth(accountID: "account-2")
    ...
    func testAccountsPageModelImportAuthDocumentAddsNewAccountWithoutChangingCurrentSelection() async throws {
        ...
        XCTAssertEqual(Set(accounts.map { $0.accountID }), Set(["account-current", "account-imported"]))
    ...
    func testSyncWorkspaceDirectorySkipsAntigravityBeforeCodexAuthExtraction() async throws {
        let now: Int64 = 1_763_216_000
        let metadataService = RecordingWorkspaceMetadataService(
            metadata: [
                WorkspaceMetadata(accountID: "workspace-2", workspaceName: "ops-space", structure: "workspace")
            ]
        )
        let storeRepository = InMemoryAccountsStoreRepository(
            store: AccountsStore(
                accounts: [
                    StoredAccount(
                        ...
                        teamAlias: nil,
                        authJSON: .object(["account_id": .string("codex-account")]),
                        addedAt: now,
                        updatedAt: now,
                        usage: nil,
                        usageError: nil
                    ...
                    StoredAccount(
                        ...
                        teamName: nil,
                        teamAlias: nil,
                        // This cannot be decoded by the OpenAI auth repository.
                        // The sync must filter it before attempting extraction.
                        authJSON: .object(["access_token": .string("antigravity-only")]),
                        addedAt: now,
                        updatedAt: now,
                        usage: nil,
                        usageError: nil,
        ...
        let coordinator = AccountsCoordinator(
            ...
            authRepository: MultiAccountAuthRepository(
                extractedByAccountID: [
                    "codex-account": makeExtractedAuth(
                        accountID: "codex-account",
                        planType: "team",
                        teamName: "primary-space"
                    ),
        ...
        XCTAssertEqual(metadataService.callCount, 1)
        XCTAssertEqual(entries.map(\.workspaceID), ["workspace-2"])
    }
    ...
    func testAntigravityExpiredCredentialFailsBeforeQuittingNativeApp() async throws {
        let now: Int64 = 1_763_216_000
        var target = makeStoredAccount(id: "ag-expired", accountID: "ag-expired", now: now)
        target.provider = .antigravity
        target.authJSON = .object([
            "access_token": .string("expired-access"),
            "refresh_token": .string("refresh"),
            "email": .string("ag-expired@example.com"),
            "antigravity_native_credential": .bool(true),
            "antigravity_native_credential_email": .string("ag-expired@example.com"),
            "expiry_date": .number(1)
        ])

        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ailsa-preflight-\(UUID().uuidString)", isDirectory: true)
        let antigravityRepository = AntigravityAuthRepository(
            paths: AntigravityAuthPaths(
                ...
                jetskiTokenPath: root.appendingPathComponent("jetski-standalone-oauth-token"),
                keyringUnavailableMarkerPath: root.appendingPathComponent("keyring-unavailable"),
                storePath: root.appendingPathComponent("legacy.json"),
        ...
        let editor = RecordingNativeProcessEditorAppService(running: true)
        let unavailableNative = AntigravityUsageService(
            nativeUsageService: AntigravityNativeUsageService(
                processListing: {
                    throw AppError.fileNotFound("not-running")
                }
            )
        )
        let storeRepository = InMemoryAccountsStoreRepository(
            store: AccountsStore(accounts: [target])
        )
        let coordinator = AccountsCoordinator(
            storeRepository: storeRepository,
            settingsRepository: TestSettingsRepository(),
            ...
            opencodeAuthSyncService: StubAILSA_SSAuthSyncService(),
            antigravityAuthRepository: antigravityRepository,
            antigravityUsageService: unavailableNative,
            dateProvider: FixedDateProvider(now: now)
        )

        do {
            try await coordinator.switchAccount(id: target.id)
            XCTFail("Expected expired native credential to fail preflight")
        } catch let error as AppError {
            guard case .unauthorized = error else {
                return XCTFail("Expected an authorization preflight error")
            }
        }

        // No quit, launch, or credential snapshot/write is permitted for a
        // deterministic invalid-target failure.
        XCTAssertEqual(editor.quitCallCount, 0)
        XCTAssertEqual(editor.launchCallCount, 0)
        XCTAssertEqual(try storeRepository.loadStore().currentAntigravityAccountID, nil)
    }

    func testLegacyAntigravityMigrationDeduplicatesByProviderAndVerifiedEmail() async throws {
        let now: Int64 = 1_763_216_000
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ailsa-migration-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let legacyPath = root.appendingPathComponent("legacy-antigravity.json")

        let email = "same-person@example.com"
        let legacyOAuth = JSONValue.object([
            "access_token": .string("legacy-access"),
            "refresh_token": .string("legacy-refresh"),
            "email": .string(email),
            "expiry_date": .number(1_800_000_000_000)
        ])
        let legacyStore = AntigravityAccountsStore(
            accounts: [
                StoredAntigravityAccount(
                    id: "legacy-card",
                    email: email,
                    label: "Legacy label",
                    oauthJSON: legacyOAuth,
                    addedAt: now - 100,
                    updatedAt: now - 50
                )
            ]
        )
        try JSONEncoder().encode(legacyStore).write(to: legacyPath)

        let preservedAuth = JSONValue.object([
            "native_session": .bool(true),
            "email": .string(email),
            "plan_type": .string("ultra")
        ])
        let existing = StoredAccount(
            id: "primary-card",
            label: "Primary label",
            email: email.uppercased(),
            accountID: "google-subject-from-old-app",
            planType: "ultra",
            teamName: nil,
            teamAlias: nil,
            authJSON: preservedAuth,
            addedAt: now - 200,
            updatedAt: now - 150,
            usage: nil,
            usageError: nil,
        ...
        let storeRepository = InMemoryAccountsStoreRepository(
            store: AccountsStore(
                accounts: [existing],
                currentAntigravityAccountID: existing.id
            )
        )
        let repository = AntigravityAuthRepository(
            paths: AntigravityAuthPaths(
                geminiDirectory: root,
                oauthCredsPath: root.appendingPathComponent("oauth.json"),
                googleAccountsPath: root.appendingPathComponent("accounts.json"),
                jetskiTokenPath: root.appendingPathComponent("native-token"),
                keyringUnavailableMarkerPath: root.appendingPathComponent("keyring-marker"),
        ...
        _ = try await coordinator.listAccounts(refreshWorkspaceMetadata: false)
        let migrated = try storeRepository.loadStore()
        XCTAssertEqual(migrated.accounts.filter { $0.provider == .antigravity }.count, 1)
        XCTAssertEqual(migrated.accounts.first?.id, existing.id)
        XCTAssertEqual(migrated.accounts.first?.authJSON, preservedAuth)
        XCTAssertEqual(migrated.accounts.first?.planType, "ultra")
        XCTAssertEqual(migrated.currentAntigravityAccountID, existing.id)
    }
}
...
private final class RecordingNativeProcessEditorAppService: EditorAppServiceProtocol, @unchecked Sendable {
...
private final class StubAILSA_SSAuthSyncService: AILSA_SSAuthSyncServiceProtocol, @unchecked Sendable {
