...
final class AccountRankingTests: XCTestCase {
    ...
    func testAutoSwitchTargetIsNilWhenCurrentHasOnePercentRemaining() {
    ...
    func testAutomaticQuotaGateUsesResetTimeBeforeTreatingAnExhaustedWindowAsAvailable() {
        ...
        let account = makeAccount(
            ...
            oneWeekResetAt: now - 1
        )

        XCTAssertTrue(AccountRanking.hasUsableQuotaForAutomaticSwitch(account, now: now))
        XCTAssertFalse(AccountRanking.isQuotaExhausted(account, now: now))
    }

    func testAutomaticQuotaGateRejectsIncompleteTwoWindowSnapshots() {
        var account = makeAccount(id: "partial", weekUsed: 20, hourUsed: 20)
        account.usage?.fiveHour = nil

        XCTAssertFalse(AccountRanking.isEligibleForAutoSwitch(account))
        XCTAssertFalse(AccountRanking.hasUsableQuotaForAutomaticSwitch(account))
    }
    ...
    private func makeAccount(
        ...
        AccountSummary(
            ...
            usage: UsageSnapshot(
                fetchedAt: 0,
                planType: nil,
                fiveHour: UsageWindow(usedPercent: hourUsed, windowSeconds: 5 * 60 * 60, resetAt: fiveHourResetAt),
                oneWeek: UsageWindow(usedPercent: weekUsed, windowSeconds: 7 * 24 * 60 * 60, resetAt: oneWeekResetAt),
                credits: nil
            ),
            usageError: nil,
            isCurrent: isCurrent
